...
from statsmodels.tsa.stattools import adfuller  # CORRECTED: Changed 'stattool' to 'stattools' and ensured 'adfuller' is correctly spelled.
...
# FIX: Convert unique years to a sorted list to enable standard list indexing (e.g., [-1], [-2])
...
# Use standard list indexing after sorting
test_start_year = all_years[-2]
test_end_year = all_years[-1]
train_end_year = all_years[-3] # The year before the test period starts
...
def train_arima(data, order, test_data_length):
...
def train_sarima(data, order, seasonal_order, test_data_length):
...
arima_order = (1, 1, 1)
arima_results, arima_predicted, arima_mae, arima_rmse, arima_mape = train_arima(train_data, arima_order, forecast_steps)

# SARIMA Model (Seasonal)
sarima_order = (1, 1, 1)
sarima_seasonal = (1, 1, 1, 12)
sarima_results, sarima_predicted, sarima_mae, sarima_rmse, sarima_mape = train_sarima(train_data, sarima_order, sarima_seasonal, forecast_steps)
...
# Model Summaries (FULL OUTPUT REQUIRED)
st.subheader("Full Model Output Summaries")

with st.expander("Show ARIMA (1,1,1) Full Statistical Summary"):
    st.text(arima_results.summary().as_text())

with st.expander("Show SARIMA (1,1,1)x(1,1,1,12) Full Statistical Summary"):
    st.text(sarima_results.summary().as_text())


# --- 3.4 Zoomed Test Period Output ---
st.subheader("2.4.1 Zoomed View: Actual vs. Predicted (Test Period)")

# Create a DataFrame for the test period comparison
test_comparison_df = pd.DataFrame({
    'Actual Sales': test_data['Sales'],
    'ARIMA Predicted': arima_predicted,
    'SARIMA Predicted': sarima_predicted
})

# Format the dataframe for display
test_comparison_display_df = test_comparison_df.copy()
test_comparison_display_df['Actual Sales'] = test_comparison_display_df['Actual Sales'].map('${:,.2f}'.format)
test_comparison_display_df['ARIMA Predicted'] = test_comparison_display_df['ARIMA Predicted'].map('${:,.2f}'.format)
test_comparison_display_df['SARIMA Predicted'] = test_comparison_display_df['SARIMA Predicted'].map('${:,.2f}'.format)
test_comparison_display_df.index = test_comparison_display_df.index.strftime('%Y-%m') # Format index

col_plot, col_table = st.columns([2, 1])

with col_plot:
    fig_zoom, ax_zoom = plt.subplots(figsize=(10, 6))
    ax_zoom.plot(test_comparison_df['Actual Sales'], label='Actual Sales (Test)', color='black', linewidth=2)
    ax_zoom.plot(test_comparison_df['ARIMA Predicted'], label='ARIMA (1,1,1) Prediction', color='#f39c12', linestyle='--')
    ax_zoom.plot(test_comparison_df['SARIMA Predicted'], label='SARIMA (1,1,1)x(1,1,1,12) Prediction', color='#2ecc71', linestyle='-.')
    ax_zoom.set_title(f'Test Period Performance ({test_data.index.min().year} - {test_data.index.max().year})')
    ax_zoom.set_xlabel('Date')
    ax_zoom.set_ylabel('Sales Value')
    ax_zoom.legend()
    ax_zoom.grid(True, alpha=0.6)
    st.pyplot(fig_zoom)

with col_table:
    st.markdown("##### Test Period Values")
    st.dataframe(test_comparison_display_df)


# --- 3.5 Final Forecast and Best Quarter ---
st.subheader("2.4.2 Combined View: Historical Data and 16-Month Forecast")
st.info("SARIMA is used for the Best Quarter analysis due to its superior handling of seasonality.")

# Train models on FULL dataset for 16-month forward forecast

@st.cache_data(show_spinner="Generating Final ARIMA Forecast...")
def generate_final_arima_forecast(data, order, steps=16):
    model = SARIMAX(data['Sales'], order=order)
    model_fit = model.fit(disp=False)
    forecast_values = model_fit.get_forecast(steps=steps).predicted_mean
    return forecast_values

@st.cache_data(show_spinner="Generating Final SARIMA Forecast...")
def generate_final_sarima_forecast(data, order, seasonal_order, steps=16):
...
final_arima_forecast_values = generate_final_arima_forecast(df, arima_order)
final_sarima_results, final_sarima_forecast_values = generate_final_sarima_forecast(df, sarima_order, sarima_seasonal)


# Forecast Plot 1 (Combined Output: Historical + Forecast)
fig_forecast, ax_forecast = plt.subplots(figsize=(10, 6))
ax_forecast.plot(df['Sales'], label='Historical Sales', color='#3498db', linewidth=2)
ax_forecast.plot(final_sarima_forecast_values, label='SARIMA Forecast (16M)', color='#2ecc71', linestyle='--')
ax_forecast.plot(final_arima_forecast_values, label='ARIMA Forecast (16M)', color='#f39c12', linestyle=':')
ax_forecast.set_title('Historical Sales and 16-Month Forward Forecast (ARIMA vs. SARIMA)')
...
# --- NEW: Zoomed Forecast Plot ---
st.subheader("2.4.3 Zoomed View: 16-Month Forward Forecast Comparison")

fig_forecast_zoom, ax_forecast_zoom = plt.subplots(figsize=(10, 6))
# Plotting only the future forecast values
ax_forecast_zoom.plot(final_sarima_forecast_values, label='SARIMA Forecast (16M)', color='#2ecc71', linestyle='--', linewidth=3)
ax_forecast_zoom.plot(final_arima_forecast_values, label='ARIMA Forecast (16M)', color='#f39c12', linestyle=':', linewidth=3)
ax_forecast_zoom.set_title('Zoomed Forecast Comparison: ARIMA vs. SARIMA (Next 16 Months)')
ax_forecast_zoom.set_xlabel('Date')
ax_forecast_zoom.set_ylabel('Sales Value')
ax_forecast_zoom.legend()
ax_forecast_zoom.grid(True, alpha=0.6)
st.pyplot(fig_forecast_zoom)


# --- Final Forecast Table ---
forecast_comparison_df = pd.DataFrame({
    'SARIMA Forecast': final_sarima_forecast_values,
    'ARIMA Forecast': final_arima_forecast_values
})

# Format the dataframe for display
forecast_comparison_display_df = forecast_comparison_df.copy()
forecast_comparison_display_df['SARIMA Forecast'] = forecast_comparison_display_df['SARIMA Forecast'].map('${:,.2f}'.format)
forecast_comparison_display_df['ARIMA Forecast'] = forecast_comparison_display_df['ARIMA Forecast'].map('${:,.2f}'.format)
forecast_comparison_display_df.index = forecast_comparison_display_df.index.strftime('%Y-%m')

st.markdown("##### 16-Month Forecast Values")
st.dataframe(forecast_comparison_display_df)


# --- Best Quarter Calculation (Using SARIMA) ---
forecast_df = pd.DataFrame({'Sales': final_sarima_forecast_values})
...
st.success(f"**Conclusion: Best Quarter (Based on SARIMA Forecast)**")
st.markdown(f"""
Based on the SARIMA model (which accounts for seasonality, giving it the lowest RMSE):
